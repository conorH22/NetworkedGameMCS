...
#include <errno.h>
 ...
 int main (int argc, char * argv [])
 {
	...
	bcopy((char *)host_info->h_addr, (char *)&server.sin_addr.s_addr, host_info->h_length); 	
 ...
 Repeat until the server terminates the connection. */
	...
	while(1){
		...
		FD_ZERO(&readset);		// Clear the fd
		FD_SET(0, &readset);	// use stdin
		...
		else if(retval == 0)
		{
			...
			printf("\nretval = %d\n", retval);
			while(count = recv(sock, i_line, LINESIZE, 0) > 0){
				/* FD_ISSET(0, &readset) will be true */
				send(1, i_line, count, 0);			
				count = recv(sock, o_line, LINESIZE, 0);
				send(sock, o_line, count, 0);
			}
